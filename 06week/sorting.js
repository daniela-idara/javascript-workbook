...
let numStr = stringsToNumbs(strNums);
...
// Find the index of the first value when added to it's index = 512 (#ATX!!)
 const atxIdx = numStr.findIndex((val, idx) => val + idx === 512);

console.log(`index: ${atxIdx}, value: ${numStr[atxIdx]}`);
...
const weatherStates = weather.map(val => val.weather_state_name).filter((val, idx, arr) => 
arr.indexOf(val) === idx);
