 ...
 const arrOfPeople = [
    {
      ...
      age: 33,
      skillSet: "artist",
      placeBorn: "Kansas City, Missouri"
    },
    {
      id: 2,
      name: "Charles Young",
   ...
   const makePlayer = (id) => {
      ...
      li.appendChild(button_red);
       ...
       const makeBluePlayer = (id) =>{
        ...
        playersBlue.style.width = '200px';
        const selectBlue = listOfPlayers.find(function(pick){  
          return pick.id == id;  
        });
        const blueIndex = listOfPlayers.indexOf(selectBlue);

         const newBluePlayer = new BlueTeammate (selectBlue.id, selectBlue.name, selectBlue.age, selectBlue.skillSet, selectBlue.placeBorn, true, true, true, true, 4, 'Hornets', 'Blue');
        blueTeam.push(newBluePlayer);
        listOfPlayers.splice(blueIndex, 1); 

         const span = document.createElement("span");
        span.appendChild(document.createTextNode(newBluePlayer.name + " : " + "Mascot - " + newBluePlayer.mascot + "; Team Color - " + newBluePlayer.teamColor ));
        playersBlue.append(span);
      }
      const makeRedPlayer = (id) =>{
        ...
        playersRed.style.width = '200px';
        const selectRed = listOfPlayers.find(function(pick){
          return pick.id == id;
        });
        const redIndex = listOfPlayers.indexOf(selectRed);

         const newRedPlayer = new RedTeammate (selectRed.id, selectRed.name, selectRed.age, selectRed.skillSet, selectRed.placeBorn, true, true, true, true, 4, 'Pigeons', 'Red');
        redTeam.push(newRedPlayer);
        listOfPlayers.splice(redIndex, 1);

        const span = document.createElement("span");
        span.appendChild(document.createTextNode(newRedPlayer.name + " : " + "Mascot - " + newRedPlayer.mascot + "; Team Color - " + newRedPlayer.teamColor));
        playersRed.append(span);
      }
      ...
      if (typeof describe === 'function'){
        ...
        it('team mate should have mascot', function(){
          let  newBluePlayer = new BlueTeammate ('newBluePlayer.id', 'newBluePlayer.name', 'newBluePlayer.age', 'newBluePlayer.skillSet', 'newBluePlayer.placeBorn', true, true, true, true, 4, 'Hornets', 'Blue');
          assert.equal(newBluePlayer.mascot, 'Hornets');
        });
      });
    }
        if (typeof describe === 'function'){
          ...
          it('team mate should have team color', function(){
            let  newRedPlayer = new RedTeammate ('newRedPlayer.id', 'newRedPlayer.name', 'newRedPlayer.age', 'newRedPlayer.skillSet', 'newRedPlayer.placeBorn', true, true, true, true, 4, 'Pigeons', 'Red');
            assert.equal(newRedPlayer.teamColor, 'Red');
          });
      });
      }